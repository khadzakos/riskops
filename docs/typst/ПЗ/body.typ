...
// Подпись по умолчанию — «Рисунок»; для `kind: table` переопределяем show-правилом ниже.
...
#let klabel(t, op) = box(
  fill: white,
  inset: (left: 1.5pt, right: 1.5pt, top: 0.5pt, bottom: 0.5pt),
  text(5.5pt)[#raw(t) #sym.space.thin #text(4.8pt, fill: rgb("#555"))[#op]],
)
...
1. периодический сбор исторических ценовых рядов финансовых инструментов из внешних источников;
2. вычисление и хранение дневных простых доходностей по каждому инструменту;
3. ведение справочника портфелей и их позиций (тикер инструмента, его весовая доля в портфеле, его совокупная цена, изменение цены портфеля);
...
7. бэктестирование качества VaR-моделей с автоматической классификацией статуса модели (OK / WARN / CRIT) на основании p-value тестов Купика и Кристофферсена;
8. стресс-тестирование портфеля по встроенным сценариям и пользовательскому параметрическому сценарию;
9. предоставление веб-интерфейса.
#v(-8mm)
\
== Краткая характеристика области применения программы

Областью применения программного комплекса является финансовая аналитика, в частности — управление рыночным риском портфелей, состоящих из биржевых инструментов. Основные пользователи системы:
...
5. реализация процедуры бэктестирования со статистическими тестами Купика (unconditional coverage) и Кристофферсена (conditional coverage);
6. реализация стресс-тестирования портфеля по историческим (2008, 2020, 1998 гг.) и параметрическим сценариям;
7. контейнеризация всех компонентов через Docker и развёртывание единой командой #raw("docker compose up -d");
...
Функциональные требования к системе сведены в @tbl-func.
...
#figure(
  table(
    ...
    [Ф-1], [Загрузка дневных цен закрытия из источников Yahoo Finance, MOEX ISS, FRED API синтетических генераторов, а также синтетических кредитных портфелей.],
    [Ф-2], [Вычисление и хранение простых дневных доходностей $r_t = (P_t - P_(t-1))/P_(t-1)$.],
    [Ф-3], [Создание, редактирование и удаление портфелей и их позиций.],
    ...
    [Ф-9], [Бэктест VaR в скользящем окне со статистическими тестами Купика и Кристофферсена.],
    [Ф-10], [Стресс-тестирование портфеля по встроенным и пользовательскому сценариям],
...
Архитектура системы построена по принципам микросервисной архитектуры с шиной событий на базе Apache Kafka и единой реляционной СУБД PostgreSQL. Общая структура взаимодействия компонентов представлена на @fig-arch.

#figure(
  canvas(length: 1cm, {
    import draw: *
    // Внешние источники
    rect((-1, 8), (3, 9), fill: c-ext, stroke: 0.5pt, radius: 1pt)
    content((1, 8.5), text(7pt)[Yahoo Finance / MOEX / FRED])
    // Frontend
    ...
    // Kafka шина (названия топиков — на стрелках публикации/подписки)
    rect((0, 3.5), (16, 4.2), fill: c-mq, stroke: 0.5pt, radius: 1pt)
    content((8, 3.85), text(7pt)[Apache Kafka (шина событий)])
    // MLflow + MinIO
    ...
    // Стрелки: внешние источники, UI, шлюз
    ...
    // --- Kafka: запись в шину (сверху вниз к y=4.2) ---
    line((0.5, 6.5), (0.5, 4.2), mark: (end: ">"), stroke: 0.55pt)
    content((0.20, 5.35), klabel("market.data.ingested", "запись"))
    line((6.5, 5), (6.5, 4.2), mark: (end: ">"), stroke: 0.55pt)
    content((6.65, 4.55), klabel("model.trained", "запись"))
    line((12.6, 6.5), (12.6, 4.2), mark: (end: ">"), stroke: 0.55pt)
    line((11, 6.5), (12.6, 6.5), stroke: 0.55pt)
    content((13.65, 5.35), klabel("portfolio.updated", "запись"))

    // --- Kafka: чтение из шины (от y=3.5 вверх к сервисам) ---
    line((2.5, 3.5), (2.5, 5.15), (4, 5.15), (4, 5.15), mark: (end: ">"), stroke: 0.55pt)
    content((2.85, 4.45), klabel("market.data.ingested", "чтение"))
    line((11.65, 3.5), (11.65, 5), mark: (end: ">"), stroke: 0.55pt)
    content((12.35, 4.18), klabel("model.trained", "чтение"))
    line((10.2, 3.5), (10.2, 4.92), (10.85, 4.92), (10.85, 5), mark: (end: ">"), stroke: 0.55pt)
    content((9.05, 4.35), klabel("portfolio.updated", "чтение"))

    // MLflow / PostgreSQL / Airflow (без Kafka)
...
- *API Gateway*: единая точка входа с фасадом, скрывающим внутреннюю топологию;
...
Состав используемых языков, фреймворков и инфраструктурных компонентов представлен в @tbl-stack.
...
#figure(
  table(
    ...
    [], [TypeScript 5 / React], [фронтенд (Next.js)],
    [Backend-фреймворки], [chi], [HTTP-роутинг в Go-сервисах],
    ...
    [], [Caddy], [HTTPS-проксирование],
    [Наблюдаемость], [Prometheus, Grafana], [метрики и визуализация],
    [Логирование], [zap (Go), logging (Python)], [структурированные логи],
    [Кодогенерация], [oapi-codegen], [генерация Go-клиентов и серверов из OpenAPI],
  ),
...
- _Python_ выбран для training-service и inference-service из-за богатой экосистемы научных библиотек (#raw("arch"), #raw("scipy"), #raw("statsmodels")), без которых реализация ML-моделей (напр. GARCH-моделей) была бы значительно более трудоёмкой.
...
Маршруты прокси-перенаправления приведены в @tbl-gateway.
...
#figure(
  table(
    ...
    [#raw("/api/portfolios")], [portfolio-service],
...
- #raw("UpsertPosition(ctx, req) -> 200|400|500") — создание/обновление позиции;
...
Полная HTTP-спецификация эндпоинтов сведена в @tbl-portfolio-rest.
...
#figure(
  table(
    ...
    [POST], [#raw("/api/portfolios/{id}/positions")], [Upsert позиции (тело: symbol, price?, quantity)],
  ...
  caption: [REST-эндпоинты Portfolio Service],
  kind: table,
) <tbl-portfolio-rest>\
...
Сервис ответственен за загрузку, нормализацию и хранение рыночных данных, а также за вычисление дневных доходностей. Поддерживаются пять источников данных, перечисленные в @tbl-sources.
...
#figure(
  table(
    ...
    [#raw("fred")],[макро-индикаторы],[FRED — это онлайн-база данных, содержащая сотни тысяч временных рядов экономических данных из множества национальных, международных, государственных и частных источников. https://fred.stlouisfed.org/],
  ...
  caption: [Источники рыночных данных],
  kind: table,
) <tbl-sources>\
...
#figure(
  ...
  caption: [REST-эндпоинты Market Data Service],
  kind: table,
) <tbl-md-rest>\
...
Список эндпоинтов приведён в @tbl-train-rest.
...
#figure(
  table(
    ...
    [POST], [#raw("/api/risk/backtest")], [ rolling-VaR backtest],
  ),
  caption: [REST-эндпоинты Training Service],
  kind: table,
) <tbl-train-rest>\

Параметры запроса #raw("POST /api/risk/train") приведены в @tbl-train-params.
...
#figure(
  ...
  caption: [Параметры запроса на обучение],
  kind: table,
) <tbl-train-params>\

#figure(
  image("ml_flow_all.png"),
  caption: [Реестр запуска обучения моделей в MLflow на примере GARCH(1,1)],
)
...
#figure(
  image("mlflow_artefacts.png"),
  caption: [Артефакты обучения инстанса модели в MLflow на примере GARCH(1,1)],
)

#figure(
  image("ml_flow_detailed_garch.png"),
  caption: [Метрики обучения модели в MLflow на примере GARCH(1,1)],
)
...
#figure(
  image("mlflow_montecarlo.png"),
  caption: [Артефакты обучения в MLflow для Monte Carlo],
)

#figure(
  image("mlflow_model_montecarlo.png"),
  caption: [Пример отслеживания метрик обучения модели Monte Carlo в MLflow],
)
...
Реализует rolling backtest. Алгоритм скользящего окна:
...
- #raw("GET /api/risk/correlation") — матрица попарных корреляций Пирсона для активов портфеля.

==== Pydantic-модели запроса прогноза

Запрос принимает идентификатор портфеля, метод расчёта (#raw("historical") / #raw("garch") / #raw("montecarlo")), уровень доверия #raw("alpha") и горизонт прогноза #raw("horizon_days").
...
Функция #raw("_store_risk_results()") выполняет атомарную запись в таблицу #raw("risk_results"). Каждый вызов инференса сохраняет несколько строк — по одной на каждую вычисленную метрику: #raw("var"), #raw("cvar"), #raw("volatility"), #raw("max_drawdown"), #raw("sharpe_ratio"), #raw("sortino_ratio"). Поля каждой строки: #raw("portfolio_id, asof_date, horizon_days, alpha, method, metric, value, model_version"). Метрики с значением #raw("None") не сохраняются. Результаты стресс-тестирования сохраняются отдельной функцией #raw("_store_stress_results()") в таблицу #raw("stress_test_results").
...
Точка входа #raw("run_scenario()") возвращает #raw("StressResult") с полями: #raw("portfolio_id, scenario_id, scenario_name, scenario_type, stressed_var, stressed_cvar, max_drawdown, worst_day, p10_return, p1_return, mean_return, n_observations, description, computed_at").

#figure(
  image("riskops_stress.png"),
  caption: [Страница «Stress» фронтенда],
)

=== Слои системы

Состав слоёв и реализующих их компонентов приведён в @tbl-layers.

#figure(
  kind: table,
  caption: [Слои системы и реализующие их компоненты],
  table(
  columns: (32mm, 1fr),
  align: (left, left),
  stroke: 0.4pt,
  ...
  [Представление], [Next.js (apps/frontend), браузерный UI, Caddy reverse proxy],
...
=== Схема базы данных
Файлы `infra/db/migrations/*.sql` содержат в себе DDL основной схемы с расширениями в дальнейших версиях.

==== Вспомогательные таблицы рыночных данных

Таблица `raw_prices` хранит сырые дневные котировки (`symbol`, `price_date`, `close`, `currency`, `source`, `ingested_at`). Таблица `processed_returns` хранит вычисленные простые дневные доходности (`symbol`, `price_date`, `ret`, `computed_at`).

==== Таблица `portfolios` и `portfolio_positions`

#raw("portfolios(id bigserial PK, name text NOT NULL UNIQUE, description text, currency text, created_at timestamptz, updated_at timestamptz)") и #raw("portfolio_positions(portfolio_id FK -> portfolios.id, symbol text, weight numeric, quantity numeric DEFAULT 0, price numeric DEFAULT 0, updated_at timestamptz, PRIMARY KEY(portfolio_id, symbol))"). Вес позиции вычисляется как `quantity × price`, после чего все веса портфеля нормируются так, чтобы их сумма равнялась 1.

==== Таблица `risk_results`

Хранит результаты расчётов риска. Структура полей приведена в @tbl-risk-result-fields.

#figure(
  kind: table,
  caption: [Структура таблицы risk_results],
  table(
  ...
  [`asof_date`], [`date NOT NULL`], [Дата расчёта],
  [`horizon_days`], [`int NOT NULL`], [Горизонт прогноза (дней)],
  [`alpha`], [`numeric(6,5) NOT NULL`], [Уровень доверия VaR],
  [`method`], [`text NOT NULL`], [Метод: historical / garch / montecarlo],
  [`metric`], [`text NOT NULL`], [Название метрики: var / cvar / volatility / max_drawdown / sharpe_ratio / sortino_ratio],
  [`value`], [`numeric(24,12) NOT NULL`], [Значение метрики],
  [`model_version`], [`text NOT NULL`], [Версия модели],
  [`created_at`], [`timestamptz NOT NULL`], [Время записи],
),
) <tbl-risk-result-fields>\

Каждый вызов инференса сохраняет несколько строк в таблицу `risk_results` — по одной строке на каждую вычисленную метрику (var, cvar, volatility, max_drawdown, sharpe_ratio, sortino_ratio). Такая нормализованная схема позволяет добавлять новые метрики без изменения структуры таблицы.

==== Прочие таблицы

Таблица `stress_test_results` хранит результаты стресс-тестирования портфелей по сценариям (стрессовые VaR/CVaR, просадки, квантили P&L, параметры сценария). Таблица `bulk_ingest_jobs` отслеживает задания массовой загрузки исторических данных (тип, источник, счётчики символов, статус, диапазон дат). Таблица `benchmark_symbols` является справочником эталонных инструментов-бенчмарков.

=== Базы данных MLflow и Airflow

Файл `infra/db/migrations/000001_mlflow_schema.sql`
содержит себе скрипт для работы нашей базы с mlflow. Он создаёт отдельную схему `mlflow` для MLflow Tracking Server. Airflow использует свою БД `airflow_db`, разворачиваемую в `docker-compose.yaml`.

#figure(
  image("db_1.png"),
  caption: [Фрагмент схемы базы данных (основная схема приложения)],
)
#figure(
  image("db_2.png"),
  caption: [Фрагмент схемы базы данных (дополнительные объекты)],
)
\
\
\
...
#figure(
  kind: table,
  caption: [Состав программных компонентов],
  table(
...
*Go-сервисы*: `chi/v5` (HTTP-роутер), `pgx/v5` (драйвер PostgreSQL). *Python-сервисы*: `fastapi`, `arch` (GARCH), `mlflow`, `aiokafka`, `sqlalchemy`. *Frontend*: `next.js`.
\
#v(-8mm)
\
== Описание API
...
#figure(
  kind: table,
  caption: [Endpoints Portfolio Service],
  table(
  ...
  [POST], [`/api/portfolios/{id}/positions`], [Upsert позиции (тело: symbol, quantity, price)],
  [DELETE], [`/api/portfolios/{id}/positions/{symbol}`], [Удаление позиции],
  [GET], [`/api/portfolios/{id}/risk/latest`], [Последние риск-метрики],
  [GET], [`/api/portfolios/{id}/risk`], [История расчётов риска (#raw("?limit=N"))],
),
) <tbl-api-portfolio-doc>\
...
#figure(
  kind: table,
  caption: [Endpoints Market Data Service],
  table(
  columns: (18mm, 60mm, 1fr),
  align: (left, left, left),
  stroke: 0.4pt,
  table.header([*Метод*], [*Путь*], [*Описание*]),
  [POST], [`/api/market-data/ingest`], [Запуск загрузки котировок по источнику и символам],
  [POST], [`/api/market-data/ingest/all`], [Загрузка по всем источникам],
  [GET], [`/api/market-data/prices`], [Запрос сырых котировок],
  [GET], [`/api/market-data/returns`], [Запрос доходностей],
  [GET], [`/api/market-data/credit`], [Кредитные данные],
  [GET], [`/api/market-data/sources`], [Список источников и их статус],
...
#figure(
  kind: table,
  caption: [Endpoints Training Service],
  table(
  columns: (18mm, 60mm, 1fr),
  align: (left, left, left),
  stroke: 0.4pt,
  table.header([*Метод*], [*Путь*], [*Описание*]),
  [POST], [`/api/risk/train`], [Запуск обучения модели VaR (асинхронно, HTTP 202)],
  [GET], [`/api/risk/train/status/{job_id}`], [Статус задания обучения],
  [GET], [`/api/risk/train/run/{run_id}`], [Детали MLflow-запуска],
  [GET], [`/api/risk/models`], [Список зарегистрированных моделей],
  [POST], [`/api/risk/backtest`], [Запуск роллингового бэктеста VaR],
),
) <tbl-api-train-doc>\
...
#figure(
  kind: table,
  caption: [Endpoints Inference Service],
  table(
  columns: (18mm, 60mm, 1fr),
  align: (left, left, left),
  stroke: 0.4pt,
  table.header([*Метод*], [*Путь*], [*Описание*]),
  [POST], [`/api/risk/predict`], [Расчёт VaR/CVaR и прочих метрик риска],
  ...
  [GET], [`/api/risk/correlation`], [Матрица корреляций активов портфеля],
),
) <tbl-api-infer-doc>\

== Описание пользовательского интерфейса

Фронтенд реализован на Next.js, TypeScript, Recharts. Файлы — `apps/frontend/src/app/`.
...
#figure(
  image("riskops_main.png"),
  caption: [Главный дашборд],
)

#figure(
  image("portfolio_page.png"),
  caption: [Страница «Portfolio»],
)
#figure(
  image("riskops_backtest.png"),
  caption: [Страница «Backtest»],
)

#figure(
  image("riskops_models.png"),
  caption: [Страница «Models»],
)
\
== Алгоритмы оценки риска

Система поддерживает три метода расчёта VaR/CVaR, реализованных в подсистеме инференса (см. раздел 4.5): историческая симуляция (эмпирические квантили взвешенных доходностей портфеля), параметрический VaR на основе GARCH(1,1) (прогноз волатильности через модель `arch`) и метод Монте-Карло GBM (симуляция траекторий цен с разложением Холецкого ковариационной матрицы). Результаты всех методов сохраняются в таблицу `risk_results` в нормализованном виде — по одной строке на метрику.
\
== Развёртывание
...
- `training_dag.py` — ежедневное переобучение моделей (22:00 UTC);
...
#figure(
  image("airflow_active.png"),
  caption: [Веб-интерфейс Apache Airflow],
)

#figure(
  image("dashboard_grafana.png"),
  caption: [Дашборд Grafana «riskops-overview»],
)
\
== Тестирование
...
+ Реализована полноценная подсистема  rolling backtesting с тестами Купика (Unconditional Coverage) и Кристофферсена (Conditional Coverage);
...
В рамках данного проекта расчёт экономической эффективности не предусмотрен.

== Предполагаемая потребность

Разрабатываемый программный комплекс может быть полезен тем, кому нужна сводная количественная оценка рыночного риска инвестиционного портфеля: загрузка и нормализация котировок, расчёт риск-метрик (VaR, CVaR и др.), регистрация и переобучение моделей при типовых для курсовой работы сценариях — без многократной ручной пересборки таблиц и скриптов. Предполагаемая потребность проявляется в учебных и исследовательских задачах по количественным финансам и MLOps и как прототип внутреннего стенда до выбора промышленного решения.

== Экономические преимущества разработки по сравнению с отечественными и зарубежными образцами или аналогами

RiskOps во многом опирается на открытый исходный код и может быть развернут на собственной инфраструктуре, доработан под новые источники рыночных данных и форматы интеграции. Для объективности сравнения в качестве аналогов взяты продукты, закреплённые в техническом задании: MSCI RiskMetrics, платформа Aladdin (BlackRock) и онлайн-сервис PortfolioVisualizer. Приведённые оценки по коммерческим продуктам носят ориентировочный характер и могут изменяться в зависимости от региона, объёма лицензии и набора модулей. Сводное сравнение приведено в @tbl-compare-analogs.

#figure(
  kind: table,
  caption: [
    Сравнительный анализ RiskOps с аналогами по параметрам, согласованным с техническим заданием; по отдельным пунктам для коммерческих продуктов указаны обобщённые оценки.
  ],
  {
    set text(size: 8pt)
    table(
      columns: (1.05fr, 1.08fr, 1.06fr, 1.06fr, 1.06fr),
      align: (left + horizon, left + horizon, left + horizon, left + horizon, left + horizon),
      inset: (x: 2.5pt, y: 2.5pt),
      stroke: 0.45pt + rgb("#ccc"),
      table.header([Параметр], [RiskOps], [MSCI RiskMetrics], [Aladdin (BlackRock)], [PortfolioVisualizer]),
      ...
      [Целевая аудитория],
      [риск-аналитики, исследователи, учебные стенды, частные инвесторы],
      [крупные банки, хедж-фонды, управляющие компании],
      [институциональные инвесторы, крупные УК],
      [частные инвесторы, финансовые консультанты],

      [Доступность исходного кода],
      [полный (микросервисы, пайплайны, UI)],
      [нет (проприетарный продукт)],
      [нет],
      [нет (веб-сервис)],

      [Развёртывание on-prem / изолированный контур],
      [да (#raw("docker compose"), внутренняя сеть)],
      [по соглашению с вендором],
      [по соглашению с вендором],
      [облако; изолированный контур ограничен],

      [Расчёт VaR],
      [да: историческая симуляция, GARCH(1,1), Monte Carlo (GBM)],
      [да, широкий набор методологий],
      [да, интегрированные модули риска],
      [в основном историческая симуляция],

      [Расчёт CVaR (ES)],
      [да],
      [да],
      [да],
      [нет / не заявлено в стандартном функционале],

      [Стресс-тестирование],
      [да: встроенные исторические и параметрические сценарии, пользовательский сценарий],
      [да, развитая методология],
      [да, институциональный уровень],
      [ограниченно (зависит от тарифа)],

      [Бэктест VaR: тесты Купика и Кристофферсена],
      [да (out-of-sample, классификация OK/WARN/CRIT)],
      [частично, зависит от конфигурации],
      [да (институциональные модули контроля моделей)],
      [нет],

      [MLOps: оркестрация и реестр моделей],
      [да: Apache Airflow, MLflow Tracking + Model Registry, MinIO],
      [нет как единого открытого контура для заказчика],
      [частично (внутренние процессы BlackRock, не «сквозной» open pipeline)],
      [нет],

      [Автоматизированный сбор рыночных данных (ingest)],
      [да: расписание, Yahoo Finance, MOEX ISS; расширяемые коллекторы],
      [поставка данных и моделей через экосистему MSCI],
      [через интеграции Aladdin Data / вендорский конвейер],
      [загрузка пользовательских рядов / данные сервиса],

      [Веб-интерфейс и REST API],
      [да: Next.js-интерфейс, единый gateway, OpenAPI],
      [веб-клиенты / отчёты по договору],
      [веб- и десктоп-клиенты экосистемы],
      [да, браузерный интерфейс],

      [Асинхронные события между сервисами (шина)],
      [да: Apache Kafka (топики #raw("portfolio.updated"), #raw("model.trained") и др.)],
      [внутренняя архитектура закрыта],
      [внутренняя архитектура закрыта],
      [не применимо (монолитный SaaS)],

      [Горячая подмена модели инференса после обучения],
      [да (подписка на Kafka, перезагрузка в inference-service)],
      [зависит от внедрения],
      [зависит от внедрения],
      [нет / обновление на стороне сервиса],

      [Совокупная стоимость владения для учебного стенда],
      [низкая (железо + электроэнергия; ПО бесплатно)],
      [высокая],
      [очень высокая],
      [низкая–средняя (подписка), но без права доработки кода],
    )
  },
) <tbl-compare-analogs>\
...
#figure(
  kind: table,
  caption: [Перечень условных обозначений],
  table(
  columns: (28mm, 1fr),
  align: (left, left),
  stroke: 0.4pt,
  ...
  [P&L], [Отчет о прибылях и убытках, показывающий финансовый результат],
  [SSR / CSR-рендеринг], [SSR (Server-Side Rendering) генерирует HTML на сервере для каждого запроса. CSR (Client-Side Rendering) отправляет пустой HTML, а отрисовка происходит в браузере через JavaScript]
),
) <tbl-abbrev>\
...
=== Цели применения модели

Целью применения генеративных моделей при выполнении курсовой работы было приведение текста программной документации к единому стилю и устранение стилистических, грамматических и пунктуационных ошибок, документирование программного кода проекта, а также при поиске ошибок связанных с работой системы.

Модель не использовалась для генерации нового содержательного материала, написания кода или создания каких-либо структурных частей.

=== Перечень применённых генеративных моделей

В рамках написания курсовой работы были использованы следующие генеративные модели:

- Claude Sonnet 4.6: #link("https://claude.ai/");
- Claude Opus 4.6: #link("https://claude.ai/").

=== Способ применения модели

Готовые фрагменты авторского текста передавались модели с запросом исправить грамматические и стилистические ошибки, улучшить читаемость и привести формулировки к академическому стилю изложения.

Готовые фрагменты авторского кода (классы и функции) передавались модели с запросом создания комментариев, описывающих функционал заданного фрагмента.

Все смысловые решения, методология, анализ и выводы являются результатом самостоятельной работы автора.
