    ...
    for attempt in range(1, _MAX_RETRIES + 1):
        try:
            ...
            logger.warning(
                ...
                attempt, _MAX_RETRIES, _RETRY_INTERVAL_S,
            )
...
def _handle_market_data_ingested(event: dict) -> None:
    """Process a single market.data.ingested event."""
    ...
    logger.info(
        "Received market.data.ingested for symbols=%s — triggering training",
        symbols,
    )
        ...
        while not stop_event.is_set():
            try:
                ...
                for _tp, messages in records.items():
                    for msg in messages:
                        try:
                            _handle_market_data_ingested(msg.value)
                        except Exception as exc:
                            logger.exception(
